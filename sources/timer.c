...
unsigned int TimerIntervalMs = 0;
...
void delayMs(uint8_t timer_num, uint32_t delayInMs)
{
  if ( timer_num == 0 )
  {
	...
	LPC_TIM0->PR  = 0x01;		/* set prescaler to zero */  // Le : changed from 0x00
  ...
  else if ( timer_num == 1 )
  {
	...
	LPC_TIM1->PR  = 0x01;		/* set prescaler to zero */
...
{
  TimerIntervalMs = (((TimerInterval + 1)) / 18000);
...
// number of millisecond since boot up,
unsigned long millis()
{
	unsigned long m;
	// should we disable irq here?
	// overflow...50 x Timerinterval days
	m = timer0_counter * TimerIntervalMs;
	return m;
}
/******************************************************************************
**                            End Of File
