...
uint8_t spiRead(uint8_t reg)
{
    ...
    SPI_transfer(_spiPortNum, reg & ~RF22_SPI_WRITE_MASK); // Send the address with the write mask off
    uint8_t val = SPI_transfer(_spiPortNum,0); // The written value is ignored, reg value is read
    digitalWrite(_slaveSelectPin, HIGH);
    return val;
}
...
{
    digitalWrite(_slaveSelectPin, LOW);
    SPI_transfer(_spiPortNum, reg | RF22_SPI_WRITE_MASK); // Send the address with the write mask on
    SPI_transfer(_spiPortNum,val); // New value follows
    digitalWrite(_slaveSelectPin, HIGH);
}
...
{
    digitalWrite(_slaveSelectPin, LOW);
    SPI_transfer(_spiPortNum, reg & ~RF22_SPI_WRITE_MASK); // Send the start address with the write mask off
    while (len--)
	*dest++ = SPI_transfer(_spiPortNum,0);
    digitalWrite(_slaveSelectPin, HIGH);
}
...
{
    digitalWrite(_slaveSelectPin, LOW);
    SPI_transfer(_spiPortNum, reg | RF22_SPI_WRITE_MASK); // Send the start address with the write mask on
    while (len--)
	SPI_transfer(_spiPortNum, *src++);
    digitalWrite(_slaveSelectPin, HIGH);
}
